...
public abstract class SoundUtil {
    ...
    public static void playSound(String sound, float volume, float pitch) {
        Minecraft.getMinecraft().getSoundHandler().playSound(new ScathaProSound(sound, volume * (float) ScathaPro.getInstance().config.getDouble(Config.Key.soundsVolume), pitch));
    }
