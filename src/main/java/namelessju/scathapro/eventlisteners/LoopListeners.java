...
public class LoopListeners
{
    ...
    public void onClientTick(ClientTickEvent event)
    {
        ...
        if (dailyStatsCheckTickTimer <= 0)
        {
            ...
            {
                MinecraftForge.EVENT_BUS.post(new RealDayStartedEvent());
            }
        ...
        if (scathaPro.variables.aprilFoolsJokeRevealTickTimer > 0)
        {
            ...
            if (scathaPro.variables.aprilFoolsJokeRevealTickTimer <= 0)
            {
                ...
                TextUtil.sendModChatMessage(EnumChatFormatting.YELLOW + "Overlay icon googly eyes permanently unlocked!\n" + EnumChatFormatting.GRAY + "You can from now on toggle them freely under " + ScathaPro.DYNAMIC_MODNAME + " Settings > Miscellaneous, as well as disable this joke for future years!");
                TextUtil.sendChatDivider();
        ...
        if (world != null)
        {
            if (scathaPro.getConfig().getBoolean(Alert.antiSleep.configKey))
            {
                scathaPro.variables.antiSleepAlertTickTimer ++;
                if (scathaPro.variables.antiSleepAlertTickTimer >= scathaPro.variables.nextAntiSleepAlertTriggerTickCount)
                {
                    if (scathaPro.variables.nextAntiSleepAlertTriggerTickCount > 0)
                    {
                        ...
                        if (!isOtherAlertPlaying) Alert.antiSleep.play();
            ...
            */
            ...
            if (scathaPro.isInCrystalHollows())
            {
                ...
                if (scathaPro.variables.lastCrystalHollowsDay != -1)
                {
                    int day = Util.getWorldDay();
                    if (day > scathaPro.variables.lastCrystalHollowsDay)
                    {
                        scathaPro.logDebug("TEST: Day increased from " + scathaPro.variables.lastCrystalHollowsDay + " to " + day);
                        if (scathaPro.variables.lastCrystalHollowsDay >= 0)
                        {
                            MinecraftForge.EVENT_BUS.post(new CrystalHollowsDayStartedEvent(day));
                        }
                        else
                        {
                            if (scathaPro.getConfig().getBoolean(Config.Key.oldLobbyAlert)
                                && OldLobbyAlertTriggerMode.isActive(scathaPro.getConfig(), OldLobbyAlertTriggerMode.ON_JOIN)
                                && day >= scathaPro.getConfig().getInt(Config.Key.oldLobbyAlertTriggerDay))
                            {
                                Alert.oldLobby.play();
                                scathaPro.variables.lastOldLobbyAlertTriggerDay = day;
                            }
                        }
                        scathaPro.variables.lastCrystalHollowsDay = day;
                    }
                }
    ...
    private void checkForDev()
    {
        ...
        if (
            ...
            (
                ...
                &&
                netHandler.getPlayerInfo("NamelessJu") != null
            )
        )
        {
