...
import namelessju.scathapro.miscellaneous.sound.ScappaSound;
...
public class DetectedWorm extends DetectedEntity
{
    ...
    public void attack(ItemStack weapon)
    {
        ...
        if (skyblockItemID != null)
        {
            ...
            if (!wasHitWithPerfectGemstoneGauntlet
                && skyblockItemID.equals("GEMSTONE_GAUNTLET"))
            {
                NBTTagCompound gemsNbt = NBTUtil.getSkyblockTagCompound(weapon, "gems");
                if (gemsNbt != null)
                {
                    boolean perfect = true;
                    for (String gemSlotId : new String[] {"JADE_0", "AMBER_0", "SAPPHIRE_0", "AMETHYST_0", "TOPAZ_0"})
                    {
                        if (!"PERFECT".equals(gemsNbt.getString(gemSlotId)))
                        {
                            perfect = false;
                            break;
                        }
