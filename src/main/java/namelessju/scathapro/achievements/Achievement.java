...
public enum Achievement
{
    ...
    worm_kills_6(AchievementCategory.KILL_AMOUNTS, "Endangered species", "Kill 25,000 worms", 25_000, Type.BONUS),
    worm_kills_7(AchievementCategory.KILL_AMOUNTS, "Very big number", "Kill 100,000 worms", 100_000, Type.BONUS),
    ...
    scatha_kills_5(AchievementCategory.KILL_AMOUNTS, "The show must go on!", "Kill 10,000 Scathas", 10_000, Type.BONUS),
    scatha_kills_repeatable(AchievementCategory.KILL_AMOUNTS, "The grind never stops", "Kill 1,000 more Scathas", 1000, Type.BONUS, true),
    ...
    scatha_pet_drop_any_repeatable(AchievementCategory.SCATHA_PET_DROPS, "Another dozen Scathas", "Drop 12 more Scatha pets of any rarity", 12, Type.BONUS, true),
    ...
    scatha_pet_drop_super_secret_setting(AchievementCategory.SCATHA_PET_DROPS, "Super Secret Scatha", "Drop a Scatha pet in a Super Secret Setting", 1, Type.SECRET),
    ...
    hard_stone_mined_1(AchievementCategory.HARD_STONE, "Rock solid", "Mined 1,000,000 hard stone", 1_000_000, Type.LEGACY),
    hard_stone_mined_2(AchievementCategory.HARD_STONE, "Stoned", "Mined 10,000,000 hard stone", 10_000_000, Type.LEGACY),
    hard_stone_mined_3(AchievementCategory.HARD_STONE, "Crystal hollowed", "Mined 100,000,000 hard stone", 100_000_000, Type.LEGACY),
    ...
    kill_perfect_gemstone_gauntlet(AchievementCategory.WORM_HIT_KILL, "In the snap of a finger", "Kill a worm using a golden Gemstone Gauntlet", 10, Type.HIDDEN),
    ...
    anomalous_desire_waste(AchievementCategory.ABILITY, "Wasteful Desire", "Waste most of the Anomalous Desire ability during the worm spawn cooldown", 1, Type.SECRET),
    anomalous_desire_recover(AchievementCategory.ABILITY, "Anomalous Recovery", "Spawn a worm with Anomalous Desire after wasting more than half of it during the worm spawn cooldown", 1, Type.SECRET),
    ...
    public void setRepeatingProgress(float startValue, float value, boolean allowUnlocking)
    {
        ...
        UnlockedAchievement unlockedAchievement = null;
        
        int goalReachedCount = (int) Math.max(progressValue / this.goal, 0f);
        if (goalReachedCount >= 1)
        {
            unlockedAchievement = ScathaPro.getInstance().getAchievementManager().unlockAchievement(this, goalReachedCount);
        }
        else unlockedAchievement = ScathaPro.getInstance().getAchievementManager().getUnlockedAchievement(this);
        ...
        setProgress(progressValue > 0f ? progressValue % this.goal : 0f, false); // note: the goal is never reached with this calculation anyways
    }
