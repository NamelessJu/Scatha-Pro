...
public class DevCommand extends CommandBase
{
    ...
    private boolean devTrigger(String trigger, String[] arguments) throws CommandException
    {
        ...
        else if (trigger.equalsIgnoreCase("scathaKills"))
        {
            if (arguments.length == 0) throw new CommandException("Missing kill count argument");
            
            int kills = CommandBase.parseInt(arguments[0], 0);
            ScathaPro.getInstance().variables.scathaKills = kills;
            ScathaPro.getInstance().getOverlay().updateScathaKills();
            ScathaPro.getInstance().getAchievementLogicManager().updateKillsAchievements();
