...
public class EntityDetectionManager
{
    ...
    private long lastBlackHolePlacedTime = -1L;
    ...
    public void reset()
    {
        ...
        lastBlackHolePlacedTime = -1;
    }

    public @Nullable DetectedEntity getDetectedEntityById(int id)
    {
    ...
    public void tick(LocalPlayer player)
    {
        ...
        for (int i = activeEntities.size() - 1; i >= 0; i --)
        {
            ...
            if (!detectedEntity.entity.isRemoved())
            {
                detectedEntity.tick();
                continue;
            }
            ...
            {
                if (detectedEntity.getMaxLifetime() >= 0 && detectedEntity.getCurrentLifetime() >= detectedEntity.getMaxLifetime() - (long) Math.min(Constants.pingThreshold, detectedEntity.getMaxLifetime() * 0.2))
                {
                ...
                {
                    boolean wasBlackHoled = false;

                    if (detectedEntity.canBeBlackHoled())
                    {
                        if (lastBlackHolePlacedTime >= 0L && now - lastBlackHolePlacedTime < 3000L)
                        {
                            wasBlackHoled = true;
                        }
        ...
        {
            if (getDetectedEntityById(armorStand.getId()) != null) continue;
    ...
    {
        if (scathaPro.minecraft.player != null && Constants.ItemID.isBlackHole(SkyBlockItemUtil.getItemID(itemStack)))
        {
            lastBlackHolePlacedTime = TimeUtil.getEpochMilliseconds();
            ScathaPro.LOGGER.debug("Black Hole item used");
        }
    }
    ...
    private void register(DetectedEntity detectedEntity)
    {
        ...
        if (!registeredEntities.containsKey(entityId))
        {
            ...
            ScathaProEvents.detectedEntityRegisteredEvent.trigger(
                new ScathaProEvents.DetectedEntityRegisteredEventData(scathaPro, detectedEntity)
            );
            if (ScathaPro.LOGGER.isDebugEnabled()) ScathaPro.LOGGER.debug(
