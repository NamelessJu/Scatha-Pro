...
public class CustomAlertModeManager
{
    ...
    {
        if (!doesSubModeExist(subModeId)) return Component.literal("(mode not found)").withStyle(Style.EMPTY.withItalic(true));

        CustomAlertModeMeta meta = subModeMetas.getOrLoad(subModeId);
        String submodeName = meta.modeName.get();
        if (submodeName == null || submodeName.isBlank())
        {
            return Component.literal("(unnamed)").withStyle(Style.EMPTY.withItalic(true));
        }
        else submodeName = submodeName.trim();
        return Component.literal(TextUtil.removeLegacyFormatting(submodeName));
    }
