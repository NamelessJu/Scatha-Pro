...
public class AchievementLogicManager
{
    ...
    public AchievementLogicManager(ScathaPro scathaPro)
    {
        ...
        scathaPro.persistentDataProfileManager.onProfileChanged(eventData -> {
            ...
            int scathaKills = eventData.profileData().scathaKills.get();
            int scathaKillsAtLastDrop = eventData.profileData().scathaKillsAtLastDrop.getOr(-1);
    ...
    {
        int lobbyWormKills = scathaPro.secondaryStatsManager.perLobbyStats.getRegularWormKills()
            + scathaPro.secondaryStatsManager.perLobbyStats.getScathaKills();
    ...
    {
        int totalWormKillsToday = scathaPro.secondaryStatsManager.perDayStats.getRegularWormKills()
            + scathaPro.secondaryStatsManager.perDayStats.getScathaKills();
    ...
    {
        int spawnStreak = scathaPro.secondaryStatsManager.perLobbyStats.getScathaSpawnStreak();
    ...
    public void handleTunnelVisionRecoverAchievement()
    {
        ...
        if (scathaPro.coreManager.tunnelVisionStartTime < 0L) return;

        long tunnelVisionElapsedTime = TimeUtil.getEpochMilliseconds() - scathaPro.coreManager.tunnelVisionStartTime;
        if (tunnelVisionElapsedTime >= 10_000 // <- prevent incorrect unlock when ability was just used after pre-spawn
            && tunnelVisionElapsedTime < Constants.tunnelVisionEffectDuration)
        {
    ...
    public void updateScathaSpawnAchievements(DetectedWorm worm)
    {
        ...
        {
            long timeSincePreviousSpawn = TimeUtil.getEpochMilliseconds() - scathaPro.coreManager.lastWormSpawnTime;
        ...
        if (player != null)
        {
            ...
            SkyBlockItemUtil.getData(helmetItem, data -> {
                if (!Util.optionalValueEquals(data.getString(SkyBlockItemUtil.KEY_ID), Constants.ItemID.pet)) return;
                data.getString(SkyBlockItemUtil.KEY_PET_INFO).ifPresent(petInfo -> {
        ...
        Component customName = worm.entity.getCustomName();
        if (customName != null)
        {
            customName.visit((style, text) -> {
                if (text.contains("Scatha"))
                {
                    ...
                    if (textColor != null && Objects.equals(textColor.getValue(), ChatFormatting.DARK_PURPLE.getColor()))
                    {
                        Achievement.scatha_spawn_runic.unlock();
                    }
