...
public abstract class LayoutScreen extends ScathaProScreen
{
    ...
    protected static class GridBuilder
    {
        ...
        public <T extends LayoutElement> T addElement(T layoutElement, int columnSpan, @Nullable LayoutSettings layoutSettings)
        {
            ...
            return grid.addChild(layoutElement, cellY + layoutElement.getY(), cellX + layoutElement.getX(), 1, columnSpan, layout);
        }
        ...
        public <T extends LayoutElement> T addSingleCell(T layoutElement)
        {
            return addSingleCell(layoutElement, true, true);
        }

        public <T extends AbstractWidget> T addSingleCellWidget(T widget)
        {
            return addSingleCell(widget, true, true);
        }

        public <T extends LayoutElement> T addSingleCell(T layoutElement, boolean overrideWidth, boolean overrideHeight)
        {
            LayoutSettings.LayoutSettingsImpl layout = grid.defaultCellSetting().getExposed();
            if (layoutElement instanceof AbstractWidget widget)
            {
                if (overrideWidth) widget.setWidth((WIDTH - (columns - 1) * (layout.paddingRight + layout.paddingLeft)) / columns);
                if (overrideHeight) widget.setHeight(20);
            }
            return addElement(layoutElement, 1, null);
        }

        public <T extends LayoutElement> T addFullWidth(T layoutElement)
        {
            return addFullWidth(layoutElement, true, true);
        }

        public <T extends AbstractWidget> T addFullWidthWidget(T widget)
        {
            return addFullWidth(widget);
        }

        public <T extends LayoutElement> T addFullWidth(T layoutElement, boolean overrideWidth, boolean overrideHeight)
        {
            if (layoutElement instanceof AbstractWidget widget)
            {
                if (overrideWidth) widget.setWidth(WIDTH);
                if (overrideHeight) widget.setHeight(20);
            }
