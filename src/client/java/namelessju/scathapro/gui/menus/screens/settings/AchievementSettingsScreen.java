        ...
        gridBuilder.addSingleCell(repeatUnlockAlertsButton = booleanConfigButton(
            ...
            _ -> Tooltip.create(
                config.achievements.playAlerts.get()
                    ? Component.literal("Whether to play unlock alerts for achievements getting repeated").withColor(TextColor.GRAY)
                    : Component.literal("Applies only when achievement unlock alerts are enabled in general").withColor(TextColor.YELLOW)
            ), null
        ));
        gridBuilder.addGap();
        gridBuilder.addFullWidth(booleanConfigButton("Pre-Open List Categories", config.achievements.listPreOpenCategories,
            _ -> Tooltip.create(
                Component.literal("Automatically opens all categories when opening the achievement list").withColor(TextColor.GRAY)
            ), null
        ));
